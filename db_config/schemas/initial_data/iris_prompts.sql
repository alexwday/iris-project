...
-- Generated: 2026-02-10T18:23:11.152544
-- 
...
DECISION FRAMEWORK (apply steps in order):

Step 1: Is the user''s INTENT unclear? → ask_clarification
   Triggers (require clarification unless conversation already specifies the aspect):
   - Very short queries (1-3 words) with no context: "Leases", "Revenue", "Adjustments"
   - Missing subject: "How does it work?", "What''s the treatment?", "What are the standards?"
   - No-context phrases: "Tell me about accounting", "I need help", "I need guidance"
   Action: Ask ONE focused clarification question.
   Only use when the query is genuinely ambiguous, missing context would lead to a completely wrong research direction, and a reasonable assumption cannot be made.

Step 2: Is intent CLEAR but scope BROAD? → request_deep_research_approval
   Triggers (user EXPLICITLY requests comprehensive results):
   ...
   NEVER use for single-word queries, vague statements, or questions without a subject — those need ask_clarification.

Step 3: Is intent clear and scope focused? → proceed_with_research (DEFAULT)
...
INSERT INTO prompts (model, layer, name, version, description, system_prompt, user_prompt, tool_definition)
VALUES ('iris', 'agent', 'direct_response', '1.0.0', 'Generates direct responses from conversation context', '<role>
...
INSERT INTO prompts (model, layer, name, version, description, system_prompt, user_prompt, tool_definition)
VALUES ('iris', 'agent', 'planner', '1.0.0', 'Selects databases for research based on research statement', '<role>
...
</instructions>', '{"type":"function","function":{"name":"select_databases","parameters":{"type":"object","required":["databases"],"properties":{"databases":{"type":"array","items":{"type":"integer","minimum":0,"description":"Database index from AVAILABLE_DATABASES"},"minItems":1,"description":"Database indices to query (most relevant)"}}},"description":"Select databases to query for the research statement.\n\nProvide database INDEX NUMBERS from AVAILABLE_DATABASES.\n\nPrefer targeted selection over broad unfocused searches."}}'::jsonb)
ON CONFLICT (model, layer, name, version) DO UPDATE SET
    description = EXCLUDED.description,
    system_prompt = EXCLUDED.system_prompt,
    user_prompt = EXCLUDED.user_prompt,
...
INSERT INTO prompts (model, layer, name, version, description, system_prompt, user_prompt, tool_definition)
VALUES ('iris', 'agent', 'router', '1.0.0', 'Routes user queries to direct response or database research', '<role>
...
- Multiple references can be grouped: ''[REF:1,2]'' or placed separately: ''[REF:1] [REF:2]''
- NEVER put [REF:X] on a line by itself or add blank lines around it

Examples of CORRECT placement:
- Paragraph: ''The standard requires five-step recognition [REF:1]. Implementation guidance is provided separately [REF:2].''
...
Approach: Low confidence response with hedging language ("The available sources provide limited guidance..."), acknowledge limitations, suggest what additional research might help.
</examples>', '<input>
...
-- Inserted/Updated 8 IRIS prompts
