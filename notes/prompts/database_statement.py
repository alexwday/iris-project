...
This module now reads database configurations from the iris_database_registry PostgreSQL table
via the DatabaseMetadataRepository. The hardcoded AVAILABLE_DATABASES dict is kept temporarily
for backward compatibility but will be removed in a future cleanup stage.
...
from .database_metadata_repo import get_repository
...
def get_database_statement() -> str:
    ...
    Now reads from iris_database_registry via DatabaseMetadataRepository.
...
"""
    ...
    internal_dbs = {
        k: v for k, v in all_databases.items() if k.startswith("internal_")
    }
    external_dbs = {
        k: v for k, v in all_databases.items() if k.startswith("external_")
    }
  ...
  <CONTENT_TYPE>{db_info.get('content_type', 'research documents')}</CONTENT_TYPE>
  <QUERY_TYPE>{db_info.get('query_type', 'semantic search')}</QUERY_TYPE>
  <USAGE>{db_info.get('use_when', db_info.get('db_description', ''))}</USAGE>
</DATABASE>
...
def get_available_databases(filtered=False):
    """
    Returns the dictionary of available databases from iris_database_registry.

    Now reads from the database via DatabaseMetadataRepository.
    AD groups and sample questions are stored in the registry table.

    Args:
        filtered: If True, removes query_type, content_type, use_when keys
    ...
    # AD group mapping from config (for runtime override if needed)
    ...
    for db_name, db_info in all_databases.items():
        ...
        # Use AD group from config override, or from registry
        db_info_api["ad_group"] = db_to_ad_group.get(db_name, db_info.get("ad_groups", [None])[0] if db_info.get("ad_groups") else None)

        # Use sample_questions from registry
