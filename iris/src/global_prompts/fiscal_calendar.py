...
def get_quarter_dates(fiscal_year: int, fiscal_quarter: int) -> Dict[str, datetime]:
    ...
    if fiscal_quarter < 1 or fiscal_quarter > 4:
        raise ValueError(f"Invalid fiscal quarter: {fiscal_quarter}. Must be between 1 and 4.")
...
def get_fiscal_statement() -> str:
    ...
    try:
        ...
        statement = f"""We are currently operating in Fiscal Year {fiscal_year} (FY{fiscal_year}), Quarter {fiscal_quarter} (Q{fiscal_quarter}), which spans {current_quarter_range}. Our fiscal year runs from November 1st through October 31st."""
