...
def setup_oauth() -> str:
    ...
    if not IS_RBC_ENV or not USE_OAUTH:
        ...
        key_preview = OPENAI_API_KEY[:TOKEN_PREVIEW_LENGTH] + "..." if len(OPENAI_API_KEY) > TOKEN_PREVIEW_LENGTH else OPENAI_API_KEY
    ...
    logger.info(f"Using client ID: {CLIENT_ID[:4]}****")  # Show only first 4 chars for security
    ...
    while attempts < MAX_RETRY_ATTEMPTS:
        ...
        try:
            logger.info(f"Attempt {attempts}/{MAX_RETRY_ATTEMPTS}: Requesting OAuth token")
            ...
            last_exception = e
            attempt_time = time.time() - attempt_start
            logger.warning(f"OAuth token request attempt {attempts} failed after {attempt_time:.2f} seconds: {str(e)}")
