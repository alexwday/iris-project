...
2.  **Vector Search**: Performs a vector similarity search against the PwC external IFRS guidance document embeddings, filtered by the specific PwC document ID.
...
This subagent is invoked synchronously by the database router when the PwC external IFRS guidance database is selected for querying. It adheres to the interface expected by the router, returning results suitable for further summarization or direct response.

The subagent strictly follows compliance and citation guidelines, ensuring all synthesized content is traceable to the source PwC external IFRS guidance database.

---

This README serves as detailed process documentation for the External PwC subagent implementation.
