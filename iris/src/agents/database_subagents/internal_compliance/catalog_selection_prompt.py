# internal_compliance/catalog_selection_prompt.py
"""
Prompt templates for selecting relevant Internal Compliance documents from the catalog.

This module contains prompts used to guide the LLM in selecting
the most relevant documents from the Internal Compliance catalog based on the user query.
"""


def get_catalog_selection_prompt(user_query: str, formatted_catalog: str) -> str:
    """
    Generate a prompt for selecting relevant documents from the Internal Compliance catalog.

    Args:
        user_query (str): The original user query
        formatted_catalog (str): The formatted catalog of Internal Compliance documents

    Returns:
        str: The formatted prompt for the LLM
    """
    prompt = f"""# TASK
You are helping to search through a catalog of internal Compliance documents (specifically related to Auditor Independence Policy FIN-ACC-214-EN) to find
