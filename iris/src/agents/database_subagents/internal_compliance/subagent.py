# internal_compliance/subagent.py
"""
Internal Compliance Subagent (Async Version)

This module handles queries to the Internal Compliance database asynchronously,
...
Functions:
    query_database_sync: Synchronously query the Internal Compliance database
"""
...
def fetch_compliance_catalog() -> List[Dict[str, Any]]:
    """
    Fetch the full internal Compliance catalog from the database synchronously.
    """
    logger.info(f"Fetching full Compliance catalog (environment: {config.ENVIRONMENT})")
    conn = connect_to_db()
    catalog_records: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for Compliance catalog")
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                FROM apg_catalog
                WHERE document_source = 'internal_compliance'
        ...
        logger.info(
            f"Retrieved {len(catalog_records)} Compliance catalog entries from database"
        )
    except Exception as e:
        logger.error(f"Error fetching Compliance catalog from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return catalog_records
...
def fetch_document_content(doc_ids: List[str]) -> List[Dict[str, Any]]:
    """
    Fetch the content of specified Compliance documents from the database synchronously.
    """
    logger.info(f"Fetching Compliance content for documents: {doc_ids}")
    if not doc_ids:
        logger.warning("No Compliance document IDs to fetch")
        return []
    conn = connect_to_db()
    result: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for Compliance content")
        return result
    try:
        doc_names = {}
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                AND document_source = 'internal_compliance'
            """,
                doc_ids,
            )
            for row in cur.fetchall():
                doc_names[row[0]] = row[1]
            logger.info(f"Found {len(doc_names)} Compliance documents for IDs: {doc_ids}")
        ...
        logger.info(f"Retrieved Compliance content for {len(result)} documents from database")
    except Exception as e:
        logger.error(f"Error fetching Compliance document content from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return result
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_compliance", # Default to compliance
    process_monitor=None, # Added process_monitor
    stage_name: Optional[str] = None # Added stage_name
) -> List[str]:
    """
    Use an LLM to select the most relevant Compliance documents from the catalog based on the query (synchronous).
    """
    logger.info("Selecting relevant Compliance documents from catalog")
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.info(f"LLM selected Compliance document IDs: {selected_ids}")
                return selected_ids
            else:
                logger.error(
            ...
            if valid_ids:
                logger.warning(
                    f"Extracted Compliance document IDs using fallback regex: {valid_ids}"
                )
                return valid_ids
            logger.error("Could not extract Compliance document IDs from response using fallback.")
            return []
    except Exception as e:
        logger.error(f"Error during LLM Compliance document selection: {str(e)}")
        return []
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for Compliance (synchronous).
    """
    logger.info(
    ...
    try:
        logger.info(
            f"Initiating Compliance Synthesis API call (DB: {database_name})"
        )  # Added contextual log
        # Direct synchronous call - now returns a tuple
...
def query_database_sync(
    ...
    """
    Synchronously query the Internal Compliance database based on the specified scope.
    ...
    """
    logger.info(
        f"Querying Internal Compliance database (sync): '{query}' with scope: {scope}"
    )
    database_name = "internal_compliance" # Set database name
    ...
    try:
        ...
        catalog = fetch_compliance_catalog() # Use compliance function
        logger.info(f"Retrieved {len(catalog)} total Compliance catalog entries")
        if not catalog:
            ...
            if scope == "metadata":
                ...
                response = {
                    "detailed_research": "No documents found in the Internal Compliance database catalog.",
        ...
        logger.info(
            f"LLM selected {len(selected_doc_ids)} relevant Compliance document IDs: {selected_doc_ids}"
        )
        ...
        if scope == "metadata":
            ...
            logger.info(
                f"Returning {len(selected_items)} selected Compliance metadata items."
            )
            ...
            documents = fetch_document_content(selected_doc_ids) # Use compliance function
            logger.info(
                f"Retrieved content for {len(documents)} Compliance documents for research."
            )
        ...
        if scope == "metadata":
            ...
            response = {
                "detailed_research": f"**Error processing request for Internal Compliance:** {str(e)}",
                "status_summary": default_error_status,
            }
