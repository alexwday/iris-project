...
"""
Internal Compliance Subagent (Sync Version)

This module handles queries to the Internal Compliance database synchronously,
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_compliance",
) -> List[str]:
    """
    Use an LLM to select the most relevant Compliance documents synchronously.
    """
    ...
    try:
        ...
        try:
            ...
            if valid_ids:
                ...
                return valid_ids
            logger.error(
                "Could not extract Compliance document IDs from response using fallback."
            )
            return []
    except Exception as e:
...
def query_database_sync(
    ...
    """
    logger.info(f"Querying Internal Compliance database (sync): '{query}' with scope: {scope}")
    ...
    try:
        ...
        catalog = fetch_compliance_catalog()
        ...
        logger.info(f"LLM selected {len(selected_doc_ids)} relevant Compliance document IDs: {selected_doc_ids}")
