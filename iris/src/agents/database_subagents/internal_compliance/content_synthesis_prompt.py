# internal_compliance/content_synthesis_prompt.py
"""
Prompt templates for synthesizing content AND status from retrieved Internal Compliance documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple Internal Compliance documents (specifically Auditor Independence Policy) and providing a status summary.
...
SUBAGENT_ROLE = "an expert research assistant specializing in analyzing internal Compliance documents (specifically Auditor Independence Policy FIN-ACC-214-EN)"

# CO-STAR Framework Components
SUBAGENT_OBJECTIVE = """
To analyze provided Compliance document sections against a user query and generate an internal research report for the Summarizer agent.
Your objective is to:
1. Determine the relevance of the provided document content (related to Auditor Independence Policy) to the user query.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    ...
    prompt_parts = [
        ...
        "<CONTEXT>",
        "You are analyzing sections from the internal Compliance database (specifically Auditor Independence Policy FIN-ACC-214-EN).",
        ...
        "</INPUT_DOCUMENTS>",
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the provided Compliance document section content (related to Auditor Independence Policy). Determine how well the content addresses the query.",
...
# Note: Internal Compliance currently only has one document, so 'individual file synthesis' might be relevant later if more are added.
