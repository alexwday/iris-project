...
"""
Internal ESG Subagent (Sync Version)

This module handles queries to the Internal ESG database synchronously,
...
def fetch_esg_catalog() -> List[Dict[str, Any]]:
    ...
    """
    logger.info(f"Fetching full ESG catalog (environment: {ENVIRONMENT})")
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_esg",
) -> List[str]:
    """
    Use an LLM to select the most relevant ESG documents synchronously.
    """
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.error(
                    f"LLM response for ESG selection was valid JSON but not list of strings: {response_str}"
                )
                return []
        except json.JSONDecodeError:
            logger.error(
                "Failed to parse ESG selection LLM response as JSON, attempting fallback"
            )
            matches = re.findall(r'"([^"]+)"', response_str)
            valid_ids = [
                m for m in matches if m.isdigit()
            ]  # Assuming ESG IDs are numeric strings
            if valid_ids:
                logger.warning(
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for ESG (synchronous).
    """
    logger.info(
...
def query_database_sync(query: str, scope: str, token: Optional[str] = None, process_monitor=None) -> SubagentResult:
    ...
    try:
        ...
        logger.info(f"LLM selected {len(selected_doc_ids)} relevant ESG document IDs: {selected_doc_ids}")
        ...
        if scope == "metadata":
            ...
            logger.info(f"Returning {len(selected_items)} selected ESG metadata items.")
