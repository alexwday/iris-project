# internal_esg/content_synthesis_prompt.py
"""
Prompt templates for synthesizing content AND status from retrieved Internal ESG documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple Internal ESG documents and providing a status summary.
...
SUBAGENT_ROLE = "an expert research assistant specializing in analyzing internal ESG (Environmental, Social, Governance) guidance documents"

# CO-STAR Framework Components
SUBAGENT_OBJECTIVE = """
To analyze provided ESG document sections against a user query and generate an internal research report for the Summarizer agent.
Your objective is to:
1. Determine the relevance of the provided document content (related to ESG Materiality and Prior Period Changes) to the user query.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    ...
    prompt_parts = [
        ...
        "<CONTEXT>",
        "You are analyzing sections from the internal ESG (Environmental, Social, Governance) guidance database.",
        ...
        "</INPUT_DOCUMENTS>",
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the provided ESG document section content. Determine how well the content addresses the query.",
...
# Note: Internal ESG has multiple documents, so this synthesis prompt is appropriate.
