...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_cheatsheets",
) -> List[str]:
    """
    Use an LLM to select the most relevant Cheatsheets documents synchronously.
    """
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.error(
                    f"LLM response for Cheatsheets selection was valid JSON but not list of strings: {response_str}"
                )
                return []
        except json.JSONDecodeError:
            logger.error(
                "Failed to parse Cheatsheets selection LLM response as JSON, attempting fallback"
            )
            matches = re.findall(r'"([^"]+)"', response_str)
            valid_ids = [
                m for m in matches if m.isdigit()
            ]  # Assuming Cheatsheets IDs are numeric strings
            if valid_ids:
                logger.warning(
