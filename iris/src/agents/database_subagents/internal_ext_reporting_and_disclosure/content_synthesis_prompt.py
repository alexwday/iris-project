# internal_ext_reporting_and_disclosure/content_synthesis_prompt.py
"""
Prompt templates for synthesizing content AND status from retrieved External Reporting and Disclosure documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple External Reporting and Disclosure documents and providing a status summary.
...
SUBAGENT_ROLE = "an expert research assistant specializing in analyzing internal External Reporting and Disclosure policy documents"

# CO-STAR Framework Components
SUBAGENT_OBJECTIVE = """
To analyze provided External Reporting and Disclosure document sections against a user query and generate an internal research report for the Summarizer agent.
Your objective is to:
1. Determine the relevance of the provided document content (related to material information disclosure, subsidiary reporting, trading revenue, auditor engagement/independence) to the user query.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    ...
    prompt_parts = [
        ...
        "<CONTEXT>",
        "You are analyzing sections from the internal External Reporting and Disclosure policy database.",
        ...
        "</INPUT_DOCUMENTS>",
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the provided External Reporting and Disclosure document section content. Determine how well the content addresses the query.",
...
# Note: Internal External Reporting and Disclosure has multiple documents, so this synthesis prompt is appropriate.
