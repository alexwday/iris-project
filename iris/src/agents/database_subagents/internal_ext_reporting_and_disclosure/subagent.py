# internal_ext_reporting_and_disclosure/subagent.py
"""
Internal External Reporting and Disclosure Subagent (Async Version)

This module handles queries to the Internal External Reporting and Disclosure database asynchronously,
...
Functions:
    query_database_sync: Synchronously query the Internal External Reporting and Disclosure database
"""
    ...
    for doc in documents:
        ...
        for page_num in sorted(pages_dict.keys()):
            ...
            for section in page_sections_list:
                ...
                logger.info(
                    f"External Reporting and Disclosure DEBUG: Formatting section - Page {page_num}, Section {section_id}, Name: '{section_name}'"
                )
                formatted_docs += (
                ...
                if section_name and not section_name.startswith("Section "):
                    ...
                    logger.info(
                        f"External Reporting and Disclosure DEBUG: Added citation note for descriptive section name: '{section_name}'"
                    )
                else:
                    logger.info(
                        f"External Reporting and Disclosure DEBUG: Generic section name detected: '{section_name}' - LLM should extract from content"
                    )
...
def fetch_ext_reporting_catalog() -> List[Dict[str, Any]]:
    """
    Fetch the full internal External Reporting and Disclosure catalog from the database synchronously.
    """
    logger.info(f"Fetching full External Reporting and Disclosure catalog (environment: {config.ENVIRONMENT})")
    conn = connect_to_db()
    catalog_records: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for External Reporting and Disclosure catalog")
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                FROM apg_catalog
                WHERE document_source = 'internal_ext_reporting_and_disclosure'
        ...
        logger.info(
            f"Retrieved {len(catalog_records)} External Reporting and Disclosure catalog entries from database"
        )
    except Exception as e:
        logger.error(f"Error fetching External Reporting and Disclosure catalog from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return catalog_records
...
def fetch_ext_reporting_content(doc_ids: List[str]) -> List[Dict[str, Any]]:
    """
    Fetch the content of specified External Reporting and Disclosure documents from the database synchronously.
    Now retrieves all page/section records with page_number, section_id, section_summary fields.
    """
    logger.info(f"Fetching External Reporting and Disclosure content for documents: {doc_ids}")
    if not doc_ids:
        logger.warning("No External Reporting and Disclosure document IDs to fetch")
        return []
    conn = connect_to_db()
    result: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for External Reporting and Disclosure content")
        return result
    try:
        doc_names = {}
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                AND document_source = 'internal_ext_reporting_and_disclosure'
            """,
                doc_ids,
            )
            for row in cur.fetchall():
                doc_names[row[0]] = row[1]
            logger.info(f"Found {len(doc_names)} External Reporting and Disclosure documents for IDs: {doc_ids}")
        ...
        logger.info(f"Retrieved External Reporting and Disclosure content for {len(result)} documents from database")
    except Exception as e:
        logger.error(f"Error fetching External Reporting and Disclosure document content from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return result
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_ext_reporting_and_disclosure",  # Default to par
    process_monitor=None,  # Added process_monitor
    stage_name: Optional[str] = None,  # Added stage_name
) -> List[str]:
    """
    Use an LLM to select the most relevant External Reporting and Disclosure documents from the catalog based on the query (synchronous).
    """
    logger.info("Selecting relevant External Reporting and Disclosure documents from catalog")
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.info(f"LLM selected External Reporting and Disclosure document IDs: {selected_ids}")
                return selected_ids
            else:
                logger.error(
            ...
            if valid_ids:
                logger.warning(
                    f"Extracted External Reporting and Disclosure document IDs using fallback regex: {valid_ids}"
                )
                return valid_ids
            logger.error(
                "Could not extract External Reporting and Disclosure document IDs from response using fallback."
            )
            return []
    except Exception as e:
        logger.error(f"Error during LLM External Reporting and Disclosure document selection: {str(e)}")
        return []
...
def synthesize_response_and_status(
    ...
    try:
        logger.info(
            f"Initiating External Reporting and Disclosure Synthesis API call (DB: {database_name})"
        )  # Added contextual log
        # Direct synchronous call - now returns a tuple
            ...
            if tool_call.function.name == SYNTHESIS_TOOL_SCHEMA["function"]["name"]:
                ...
                try:
                    ...
                    if all(key in arguments for key in required_keys):
                        ...
                        logger.info(
                            f"External Reporting and Disclosure DEBUG: Generated detailed_research length: {len(research)} characters"
                        )
                        if "***Source:" in research:
                            citation_count = research.count("***Source:")
                            logger.info(
                                f"External Reporting and Disclosure DEBUG: Found {citation_count} citations in detailed_research"
                            )
                            ...
                            for i, citation in enumerate(
                                ...
                                logger.info(f"External Reporting and Disclosure DEBUG: Citation {i+1}: {citation}")
                        else:
                            logger.warning(
                                "External Reporting and Disclosure DEBUG: No citations found in detailed_research - this may be the issue!"
                            )
...
def query_database_sync(
    ...
    """
    Synchronously query the Internal External Reporting and Disclosure database based on the specified scope.
    ...
    logger.info(f"Querying Internal External Reporting and Disclosure database (sync): '{query}' with scope: {scope}")
    database_name = "internal_ext_reporting_and_disclosure"  # Set database name
    ...
    try:
        ...
        catalog = fetch_ext_reporting_catalog()  # Use par function
        logger.info(f"Retrieved {len(catalog)} total External Reporting and Disclosure catalog entries")
        if not catalog:
            ...
            if scope == "metadata":
                ...
                response = {
                    "detailed_research": "No documents found in the Internal External Reporting and Disclosure database catalog.",
        ...
        logger.info(
            f"LLM selected {len(selected_doc_ids)} relevant External Reporting and Disclosure document IDs: {selected_doc_ids}"
        )
        ...
        if scope == "metadata":
            ...
            documents = fetch_ext_reporting_content(selected_doc_ids)  # Use par function
            logger.info(
                f"Retrieved content for {len(documents)} External Reporting and Disclosure documents for research."
            )
            ...
            logger.info(
                f"External Reporting and Disclosure DEBUG: Raw research_result keys: {list(research_result.keys())}"
            )
            logger.info(f"External Reporting and Disclosure DEBUG: Raw page_numbers from tool: {page_numbers}")
            logger.info(
                f"External Reporting and Disclosure DEBUG: Raw section_ids_by_page from tool: {section_ids_by_page_str}"
            )
            ...
            section_content_map = {}
            logger.info(
                f"External Reporting and Disclosure DEBUG: Retrieved {len(documents)} documents from database"
            )
            for doc in documents:
                logger.info(f"External Reporting and Disclosure DEBUG: Document keys: {list(doc.keys())}")
                page_sections = doc.get("page_sections", [])
                sections = doc.get("sections", [])  # Check old format too
                logger.info(
                    f"External Reporting and Disclosure DEBUG: Document '{doc.get('document_name', 'Unknown')}' has {len(page_sections)} page_sections and {len(sections)} old sections"
                )
                if page_sections:
                    logger.info(
                        f"External Reporting and Disclosure DEBUG: First page_section sample: {page_sections[0]}"
                    )
                elif sections:
                    logger.info(f"External Reporting and Disclosure DEBUG: First old section sample: {sections[0]}")
                else:
                    logger.info(f"External Reporting and Disclosure DEBUG: Document has no sections at all!")
                for section in page_sections:
            ...
            logger.info(
                f"External Reporting and Disclosure DEBUG: Final section_content_map keys: {list(section_content_map.keys())}"
            )
            ...
            logger.error(f"Invalid scope provided to internal_ext_reporting_and_disclosure subagent: {scope}")
            raise ValueError(f"Invalid scope: {scope}")  # Let the error propagate

    except Exception as e:
        error_msg = f"Error querying Internal External Reporting and Disclosure database (scope: {scope}): {str(e)}"
        logger.error(error_msg, exc_info=True)
        if scope == "metadata":
            response = []
        else:
            response = {
                "detailed_research": f"**Error processing request for Internal External Reporting and Disclosure:** {str(e)}",
                "status_summary": default_error_status,
            }
