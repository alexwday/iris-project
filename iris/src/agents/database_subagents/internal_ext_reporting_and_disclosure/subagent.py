...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_ext_reporting_and_disclosure", # Default to ext_reporting
    ...
    try:
        ...
        try:
            ...
            if valid_ids:
                ...
                return valid_ids
            logger.error("Could not extract External Reporting and Disclosure document IDs from response using fallback.")
            return []
    except Exception as e:
...
def query_database_sync(
    ...
    """
    logger.info(
        f"Querying Internal External Reporting and Disclosure database (sync): '{query}' with scope: {scope}"
    )
    ...
    try:
        ...
        catalog = fetch_ext_reporting_catalog() # Use ext_reporting function
        ...
        if scope == "metadata":
            ...
            documents = fetch_document_content(selected_doc_ids) # Use ext_reporting function
            logger.info(
