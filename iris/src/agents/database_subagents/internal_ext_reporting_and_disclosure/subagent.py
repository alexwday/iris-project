...
"""
Internal External Reporting and Disclosure Subagent (Sync Version)

This module handles queries to the Internal External Reporting and Disclosure database synchronously,
...
def fetch_ext_reporting_catalog() -> List[Dict[str, Any]]:
    ...
    """
    logger.info(f"Fetching full External Reporting and Disclosure catalog (environment: {ENVIRONMENT})")
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_ext_reporting_and_disclosure",
) -> List[str]:
    """
    Use an LLM to select the most relevant External Reporting and Disclosure documents synchronously.
    """
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.error(
                    f"LLM response for External Reporting and Disclosure selection was valid JSON but not list of strings: {response_str}"
                )
                return []
        except json.JSONDecodeError:
            logger.error(
                "Failed to parse External Reporting and Disclosure selection LLM response as JSON, attempting fallback"
            )
...
# Define the tool schema for research synthesis (Copied from internal_wiki)
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for External Reporting and Disclosure (synchronous).
    """
    logger.info(
...
def query_database_sync(
    ...
    try:
        ...
        catalog = fetch_ext_reporting_catalog()
        ...
        logger.info(f"LLM selected {len(selected_doc_ids)} relevant External Reporting and Disclosure document IDs: {selected_doc_ids}")
        ...
        if scope == "metadata":
            ...
            logger.info(f"Returning {len(selected_items)} selected External Reporting and Disclosure metadata items.")
