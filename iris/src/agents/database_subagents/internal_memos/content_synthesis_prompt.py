...
"""
Prompt templates for synthesizing content AND status from retrieved Memos documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple Memos documents and providing a status summary.
...
SUBAGENT_OBJECTIVE = """
To analyze provided Memos document sections against a user query and generate an internal research report for the Summarizer agent.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    """
    Generate a prompt for synthesizing content AND status from retrieved Memos documents.

    Args:
        user_query (str): The original user query from the research statement
        formatted_documents (str): The formatted content of retrieved Memos document sections
    ...
    prompt_parts = [
        ...
        "</INPUT_DOCUMENTS>",
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the provided Memos document section content. Determine how well the content addresses the query.",
