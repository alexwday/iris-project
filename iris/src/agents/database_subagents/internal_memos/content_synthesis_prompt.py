# internal_memos/content_synthesis_prompt.py
"""
Prompt templates for synthesizing content AND status from retrieved Internal Memos documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple Internal Memos documents and providing a status summary.
...
SUBAGENT_ROLE = "an expert research assistant specializing in analyzing internal Memos documents"

# CO-STAR Framework Components
SUBAGENT_OBJECTIVE = """
To analyze provided Internal Memos document sections against a user query and generate an internal research report for the Summarizer agent.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    """
    Generate a prompt for synthesizing content AND status from retrieved Internal Memos documents.

    Args:
        user_query (str): The original user query from the research statement
        formatted_documents (str): The formatted content of retrieved Internal Memos document sections
    ...
    prompt_parts = [
        ...
        "<CONTEXT>",
        "You are analyzing sections from the Internal Memos database.",
        ...
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the provided Internal Memos document section content. Determine how well the content addresses the query.",
...
# Note: Internal Memos doesn't seem to have an 'individual file synthesis' prompt.
