# external_kpmg/content_synthesis_prompt.py
"""
Prompt templates for synthesizing content AND status from retrieved KPMG guidance documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple KPMG guidance context cards and providing a status summary.
...
SUBAGENT_ROLE = "an expert research assistant specializing in analyzing external KPMG accounting guidance documents"

# CO-STAR Framework Components
SUBAGENT_OBJECTIVE = """
To analyze provided KPMG guidance context cards against a user query and generate an internal research report optimized for the Summarizer agent.
...
SYNTHESIS_TOOL_SCHEMA = {
    ...
    "function": {
        ...
        "description": "Synthesizes research findings from provided KPMG guidance context cards and generates a status summary.",
        "parameters": {
            "type": "object",
            "properties": {
                "status_summary": {
                    "type": "string",
                    "description": "Concise status summary flag (✅, ℹ️, 📄, ⚠️, ❓) indicating finding relevance based *only* on the provided KPMG guidance context cards.",
                },
                "detailed_research_report": { # Renamed from detailed_research
                    "type": "string",
                    "description": "Detailed, structured markdown report synthesizing information *only* from KPMG guidance context cards, including mandatory inline citations (Chapter, Section Title/Hierarchy, Standard, Standard Codes). Optimized for the Summarizer Agent.",
                },
            },
...
def get_content_synthesis_prompt(query: str, formatted_cards: str) -> str:
    ...
    prompt_parts = [
        ...
        "<CONTEXT>",
        "You are analyzing context cards derived from external KPMG accounting guidance documents.",
        "Each card represents a relevant piece of text from the source KPMG guidance document.",
        ...
        "</AUDIENCE>",
        "<TASK>",
        "Your goal is to provide BOTH a concise status summary flag AND a detailed, structured internal research report based *only* on the provided KPMG guidance context cards, formatted for the Summarizer Agent.",
        "<INPUT_DOCUMENTS>",
        f"<USER_QUERY>{query}</USER_QUERY>",
        f"<CONTEXT_CARDS>{formatted_cards}</CONTEXT_CARDS>",
        "</INPUT_DOCUMENTS>",
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the 'Content' field within the provided KPMG guidance context cards. Determine how well the content addresses the query.",
        ...
        "   * `✅ Found information directly addressing the query in the KPMG cards.`",
        "   * `ℹ️ Found related contextual information in the KPMG cards, but not a direct answer.`",
        "   * `📄 KPMG cards found, but they do not contain relevant information for this query.`",
        "   * `⚠️ Conflicting information found across KPMG cards.` (Explain conflicts in the detailed report)",
        "   * `❓ Query is ambiguous based on KPMG card content.` (Explain ambiguity in the detailed report)",
