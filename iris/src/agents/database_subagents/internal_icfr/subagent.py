...
def fetch_document_content(doc_ids: List[str]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Retrieved ICFR content for {len(result)} documents from database")
    except Exception as e:
        logger.error(f"Error fetching ICFR document content from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return result
...
def synthesize_response_and_status(
    ...
    try:
        logger.info(
            f"Initiating ICFR Synthesis API call (DB: {database_name})"
        )  # Added contextual log
        # Direct synchronous call
