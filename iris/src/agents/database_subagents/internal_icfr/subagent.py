...
def fetch_icfr_catalog() -> List[Dict[str, Any]]:
    ...
    """
    logger.info(f"Fetching full ICFR catalog (environment: {config.ENVIRONMENT})")
    conn = connect_to_db()
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_icfr", # Default to icfr
    process_monitor=None, # Added process_monitor
    stage_name: Optional[str] = None # Added stage_name
) -> List[str]:
    """
    Use an LLM to select the most relevant ICFR documents from the catalog based on the query (synchronous).
    """
    ...
    try:
        ...
        try:
            ...
            if valid_ids:
                ...
                return valid_ids
            logger.error("Could not extract ICFR document IDs from response using fallback.")
            return []
    except Exception as e:
...
def query_database_sync(
    ...
    database_name = "internal_icfr" # Set database name
    ...
    try:
        ...
        catalog = fetch_icfr_catalog() # Use icfr function
        ...
        if scope == "metadata":
            ...
            documents = fetch_document_content(selected_doc_ids) # Use icfr function
            logger.info(
