...
def fetch_icfr_catalog(query: str) -> List[Dict[str, Any]]:
    ...
    """
    logger.info(
        f"Fetching ICFR catalog with query: {query} (environment: {ENVIRONMENT})"
    )
...
def synthesize_response(
    ...
    try:
        ...
        error_msg = (
            f"Error synthesizing response from internal ICFR documents: {str(e)}"
        )
    ...
    def response_generator():
        try:
            ...
            doc_ids = select_relevant_documents(
                query, catalog, token, database_name="internal_icfr"
            )
            ...
            response_stream = synthesize_response(
                query, documents, token, stream=True, database_name="internal_icfr"
            )
