# internal_icfr/subagent.py
"""
Internal ICFR Subagent (Async Version)

This module handles queries to the Internal ICFR database asynchronously,
...
Functions:
    query_database_sync: Synchronously query the Internal ICFR database
"""
...
def fetch_icfr_catalog() -> List[Dict[str, Any]]:
    """
    Fetch the full internal ICFR catalog from the database synchronously.
    """
    logger.info(f"Fetching full ICFR catalog (environment: {ENVIRONMENT})")
    conn = connect_to_db(ENVIRONMENT)
    catalog_records: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for ICFR catalog")
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                FROM apg_catalog
                WHERE document_source = 'internal_icfr'
        ...
        logger.info(
            f"Retrieved {len(catalog_records)} ICFR catalog entries from database"
        )
    except Exception as e:
        logger.error(f"Error fetching ICFR catalog from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return catalog_records
...
def fetch_document_content(doc_ids: List[str]) -> List[Dict[str, Any]]:
    """
    Fetch the content of specified ICFR documents from the database synchronously.
    """
    logger.info(f"Fetching ICFR content for documents: {doc_ids}")
    if not doc_ids:
        logger.warning("No ICFR document IDs to fetch")
        return []
    conn = connect_to_db(ENVIRONMENT)
    result: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for ICFR content")
        return result
    try:
        doc_names = {}
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                AND document_source = 'internal_icfr'
            """,
                doc_ids,
            )
            for row in cur.fetchall():
                doc_names[row[0]] = row[1]
            logger.info(f"Found {len(doc_names)} ICFR documents for IDs: {doc_ids}")
        ...
        logger.info(
            f"Retrieved ICFR content for {len(result)} documents from database"
        )
    except Exception as e:
        logger.error(
            f"Error fetching ICFR document content from database: {str(e)}"
        )
    finally:
        if conn:
            conn.close()
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_icfr",
) -> List[str]:
    """
    Use an LLM to select the most relevant ICFR documents synchronously.
    """
    logger.info("Selecting relevant ICFR documents from catalog")
    ...
    try:
        logger.info(f"Initiating ICFR Document Selection API call (DB: {database_name})") # Added contextual log
        # Direct synchronous call
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.info(f"LLM selected ICFR document IDs: {selected_ids}")
                return selected_ids
            else:
                logger.error(
                    f"LLM response for ICFR selection was valid JSON but not list of strings: {response_str}"
                )
                return []
        except json.JSONDecodeError:
            logger.error(
                "Failed to parse ICFR selection LLM response as JSON, attempting fallback"
            )
            matches = re.findall(r'"([^"]+)"', response_str)
            valid_ids = [
                m for m in matches if m.isdigit()
            ]  # Assuming ICFR IDs are numeric strings
            if valid_ids:
                logger.warning(
                    f"Extracted ICFR document IDs using fallback regex: {valid_ids}"
                )
                return valid_ids
            logger.error(
                "Could not extract ICFR document IDs from response using fallback."
            )
            return []
    except Exception as e:
        logger.error(f"Error during LLM ICFR document selection: {str(e)}")
        return []
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for ICFR (synchronous).
    """
    logger.info(
    ...
    try:
        logger.info(f"Initiating ICFR Synthesis API call (DB: {database_name})") # Added contextual log
        # Direct synchronous call
...
def query_database_sync(
    ...
    """
    Synchronously query the Internal ICFR database based on the specified scope.
    """
    logger.info(
        f"Querying Internal ICFR database (sync): '{query}' with scope: {scope}"
    )
    ...
    try:
        ...
        catalog = fetch_icfr_catalog()
        logger.info(f"Retrieved {len(catalog)} total ICFR catalog entries")
        if not catalog:
            if scope == "metadata":
                return []
            else:
                return {
                    "detailed_research": "No documents found in the Internal ICFR database catalog.",
        ...
        logger.info(
            f"LLM selected {len(doc_ids)} relevant ICFR document IDs: {doc_ids}"
        )
        if not doc_ids:
            if scope == "metadata":
                return []
        ...
        if scope == "metadata":
            ...
            logger.info(
                f"Returning {len(selected_items)} selected ICFR metadata items."
            )
            ...
            logger.info(
                f"Retrieved content for {len(documents)} ICFR documents for research."
            )
        ...
        error_msg = f"Error querying Internal ICFR database (scope: {scope}): {str(e)}"
        logger.error(error_msg, exc_info=True)
        if scope == "metadata":
            return []
        else:
            return {
                "detailed_research": f"**Error processing request for Internal ICFR:** {str(e)}",
                "status_summary": default_error_status,
            }
