...
"""
Internal ICFR Subagent

This module handles queries to the Internal ICFR database, including catalog retrieval,
document selection, content retrieval, and response synthesis.

Functions:
    query_database: Query the Internal ICFR database
"""
...
def fetch_icfr_catalog(query: str) -> List[Dict[str, Any]]:
    """
    Fetch and filter the internal ICFR catalog from the database.
    ...
    """
    logger.info(f"Fetching ICFR catalog with query: {query} (environment: {ENVIRONMENT})")
    ...
    try:
        # Create SQL query to get catalog entries for internal_icfr
...
def select_relevant_documents(query: str, catalog: List[Dict[str, Any]], 
                              token: Optional[str] = None, database_name: str = "internal_icfr") -> List[str]:
    """
...
def synthesize_response(query: str, documents: List[Dict[str, Any]], 
                       ...
                       stream: bool = False, database_name: str = "internal_icfr") -> Union[str, Generator[str, None, None]]:
    """
    ...
    if not documents:
        error_msg = "No relevant information found in the internal ICFR database. Please try refining your query or check other databases."
        if stream:
    ...
    try:
        ...
        error_msg = f"Error synthesizing response from internal ICFR documents: {str(e)}"
...
def query_database(query: str, token: Optional[str] = None) -> Union[str, Generator[str, None, None]]:
    """
    Query the Internal ICFR database.
    
    This function implements the full pipeline for querying the internal ICFR:
    1. Fetch and filter the ICFR catalog
    ...
    Returns:
        Union[str, Generator[str, None, None]]: Query results from the ICFR database,
    ...
    def response_generator():
        try:
            ...
            catalog = fetch_icfr_catalog(query)
            logger.info(f"Retrieved {len(catalog)} catalog entries")
            
            if not catalog:
                yield "No documents found in the Internal ICFR database. Please try a different query or another database."
                return
            
            # Step 2: Select relevant documents using LLM (non-streaming for tool calls)
            # Pass database name for tracking
            doc_ids = select_relevant_documents(query, catalog, token, database_name="internal_icfr")
            logger.info(f"Selected {len(doc_ids)} relevant documents")
            
            if not doc_ids:
                yield "No relevant documents found in the Internal ICFR database. Please try refining your query or check other databases."
                return
            ...
            # Pass database name for tracking
            response_stream = synthesize_response(query, documents, token, stream=True, database_name="internal_icfr")
