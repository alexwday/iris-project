...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_cheatsheets", # Default to cheatsheets
    ...
    try:
        ...
        try:
            ...
            if valid_ids:
                ...
                return valid_ids
            logger.error("Could not extract Cheatsheets document IDs from response using fallback.")
            return []
    except Exception as e:
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for Cheatsheets (synchronous).
    """
    logger.info(
...
def query_database_sync(
    ...
    """
    logger.info(
        f"Querying Internal Cheatsheets database (sync): '{query}' with scope: {scope}"
    )
    ...
    try:
        ...
        catalog = fetch_cheatsheets_catalog() # Use cheatsheets function
        ...
        if scope == "metadata":
            ...
            logger.info(
                f"Returning {len(selected_items)} selected Cheatsheets metadata items."
            )
            ...
            documents = fetch_document_content(selected_doc_ids) # Use cheatsheets function
            logger.info(
