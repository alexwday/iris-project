    ...
    for doc in documents:
        ...
        for page_num in sorted(pages_dict.keys()):
            ...
            for section in page_sections_list:
                ...
                logger.info(
                    f"Wiki DEBUG: Formatting section - Page {page_num}, Section {section_id}, Name: '{section_name}'"
                )
                formatted_docs += (
                ...
                if section_name and not section_name.startswith("Section "):
                    ...
                    logger.info(
                        f"Wiki DEBUG: Added citation note for descriptive section name: '{section_name}'"
                    )
                else:
                    logger.info(
                        f"Wiki DEBUG: Generic section name detected: '{section_name}' - LLM should extract from content"
                    )
...
def fetch_wiki_catalog() -> List[Dict[str, Any]]:
    ...
    """
    logger.info(f"Fetching full Wiki catalog (environment: {config.ENVIRONMENT})")
    conn = connect_to_db()
    catalog_records: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for Wiki catalog")
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(catalog_records)} Wiki catalog entries from database"
        )
    except Exception as e:
        logger.error(f"Error fetching Wiki catalog from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return catalog_records
...
def fetch_wiki_content(doc_ids: List[str]) -> List[Dict[str, Any]]:
    """
    Fetch the content of specified Wiki documents from the database synchronously.
    Now retrieves all page/section records with page_number, section_id, section_summary fields.
    """
    logger.info(f"Fetching Wiki content for documents: {doc_ids}")
    if not doc_ids:
        logger.warning("No Wiki document IDs to fetch")
        return []
    conn = connect_to_db()
    result: List[Dict[str, Any]] = []
    if not conn:
        logger.error("Failed to connect to database for Wiki content")
        return result
    try:
        doc_names = {}
        with conn.cursor() as cur:
            ...
            logger.info(f"Found {len(doc_names)} Wiki documents for IDs: {doc_ids}")
        ...
        logger.info(f"Retrieved Wiki content for {len(result)} documents from database")
    except Exception as e:
        logger.error(f"Error fetching Wiki document content from database: {str(e)}")
    finally:
        if conn:
            conn.close()
    return result
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_wiki",  # Default to par
    process_monitor=None,  # Added process_monitor
    stage_name: Optional[str] = None,  # Added stage_name
) -> List[str]:
    """
    Use an LLM to select the most relevant Wiki documents from the catalog based on the query (synchronous).
    """
    logger.info("Selecting relevant Wiki documents from catalog")
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.info(f"LLM selected Wiki document IDs: {selected_ids}")
                return selected_ids
            else:
                logger.error(
            ...
            if valid_ids:
                logger.warning(
                    f"Extracted Wiki document IDs using fallback regex: {valid_ids}"
                )
                return valid_ids
            logger.error(
                "Could not extract Wiki document IDs from response using fallback."
            )
            return []
    except Exception as e:
        logger.error(f"Error during LLM Wiki document selection: {str(e)}")
        return []
            ...
            if tool_call.function.name == SYNTHESIS_TOOL_SCHEMA["function"]["name"]:
                ...
                try:
                    ...
                    if all(key in arguments for key in required_keys):
                        ...
                        logger.info(
                            f"Wiki DEBUG: Generated detailed_research length: {len(research)} characters"
                        )
                        if "***Source:" in research:
                            citation_count = research.count("***Source:")
                            logger.info(
                                f"Wiki DEBUG: Found {citation_count} citations in detailed_research"
                            )
                            ...
                            for i, citation in enumerate(
                                ...
                                logger.info(f"Wiki DEBUG: Citation {i+1}: {citation}")
                        else:
                            logger.warning(
                                "Wiki DEBUG: No citations found in detailed_research - this may be the issue!"
                            )
...
def query_database_sync(
    ...
    logger.info(f"Querying Internal Wiki database (sync): '{query}' with scope: {scope}")
    database_name = "internal_wiki"  # Set database name
    ...
    try:
        ...
        catalog = fetch_wiki_catalog()  # Use par function
        ...
        if scope == "metadata":
            ...
            documents = fetch_wiki_content(selected_doc_ids)  # Use par function
            logger.info(
            ...
            logger.info(
                f"Wiki DEBUG: Raw research_result keys: {list(research_result.keys())}"
            )
            logger.info(f"Wiki DEBUG: Raw page_numbers from tool: {page_numbers}")
            logger.info(
                f"Wiki DEBUG: Raw section_ids_by_page from tool: {section_ids_by_page_str}"
            )
            ...
            section_content_map = {}
            logger.info(
                f"Wiki DEBUG: Retrieved {len(documents)} documents from database"
            )
            for doc in documents:
                logger.info(f"Wiki DEBUG: Document keys: {list(doc.keys())}")
                page_sections = doc.get("page_sections", [])
                sections = doc.get("sections", [])  # Check old format too
                logger.info(
                    f"Wiki DEBUG: Document '{doc.get('document_name', 'Unknown')}' has {len(page_sections)} page_sections and {len(sections)} old sections"
                )
                if page_sections:
                    logger.info(
                        f"Wiki DEBUG: First page_section sample: {page_sections[0]}"
                    )
                elif sections:
                    logger.info(f"Wiki DEBUG: First old section sample: {sections[0]}")
                else:
                    logger.info(f"Wiki DEBUG: Document has no sections at all!")
                for section in page_sections:
            ...
            logger.info(
                f"Wiki DEBUG: Final section_content_map keys: {list(section_content_map.keys())}"
            )
