...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_management_reporting", # Default to management_reporting
    ...
    try:
        ...
        try:
            ...
            if valid_ids:
                ...
                return valid_ids
            logger.error("Could not extract Management Reporting document IDs from response using fallback.")
            return []
    except Exception as e:
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for Management Reporting (synchronous).
    """
    logger.info(
...
def query_database_sync(
    ...
    """
    logger.info(
        f"Querying Internal Management Reporting database (sync): '{query}' with scope: {scope}"
    )
    ...
    try:
        ...
        catalog = fetch_management_reporting_catalog() # Use management_reporting function
        ...
        if scope == "metadata":
            ...
            logger.info(
                f"Returning {len(selected_items)} selected Management Reporting metadata items."
            )
            ...
            documents = fetch_document_content(selected_doc_ids) # Use management_reporting function
            logger.info(
