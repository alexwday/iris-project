...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_aio", # Default to aio
    ...
    try:
        ...
        try:
            ...
            if valid_ids:
                ...
                return valid_ids
            logger.error("Could not extract AIO document IDs from response using fallback.")
            return []
    except Exception as e:
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for AIO (synchronous).
    """
    logger.info(
...
def query_database_sync(
    ...
    """
    logger.info(
        f"Querying Internal AIO database (sync): '{query}' with scope: {scope}"
    )
    ...
    try:
        ...
        catalog = fetch_aio_catalog() # Use aio function
        ...
        if scope == "metadata":
            ...
            logger.info(
                f"Returning {len(selected_items)} selected AIO metadata items."
            )
            ...
            documents = fetch_document_content(selected_doc_ids) # Use aio function
            logger.info(
