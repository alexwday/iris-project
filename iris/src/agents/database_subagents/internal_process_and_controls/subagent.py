...
"""
Internal Process and Controls Subagent (Sync Version)

This module handles queries to the Internal Process and Controls database synchronously,
...
def fetch_process_controls_catalog() -> List[Dict[str, Any]]:
    ...
    """
    logger.info(f"Fetching full Process and Controls catalog (environment: {ENVIRONMENT})")
...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_process_and_controls",
) -> List[str]:
    """
    Use an LLM to select the most relevant Process and Controls documents synchronously.
    """
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.error(
                    f"LLM response for Process and Controls selection was valid JSON but not list of strings: {response_str}"
                )
                return []
        except json.JSONDecodeError:
            logger.error(
                "Failed to parse Process and Controls selection LLM response as JSON, attempting fallback"
            )
...
def synthesize_response_and_status(
    ...
    """
    Use an LLM tool call to synthesize a detailed research response AND status summary for Process and Controls (synchronous).
    """
    logger.info(
...
def query_database_sync(query: str, scope: str, token: Optional[str] = None, process_monitor=None) -> SubagentResult:
    ...
    try:
        ...
        catalog = fetch_process_controls_catalog()
        ...
        logger.info(f"LLM selected {len(selected_doc_ids)} relevant Process and Controls document IDs: {selected_doc_ids}")
        ...
        if scope == "metadata":
            ...
            logger.info(f"Returning {len(selected_items)} selected Process and Controls metadata items.")
