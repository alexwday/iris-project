...
2.  **Vector Search**: Performs a vector similarity search against the EY external IFRS guidance document embeddings, optionally filtered by document ID.
...
This subagent is invoked synchronously by the database router when the EY external IFRS guidance database is selected for querying. It adheres to the interface expected by the router, returning results suitable for further summarization or direct response.

The subagent strictly follows compliance and citation guidelines, ensuring all synthesized content is traceable to the source EY external IFRS guidance database.

---

This README serves as detailed process documentation for the External EY subagent implementation.
