# internal_global_finance_standards/content_synthesis_prompt.py
"""
Prompt templates for synthesizing content AND status from retrieved Global Finance Standards documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple Global Finance Standards documents and providing a status summary.
...
SUBAGENT_ROLE = "an expert research assistant specializing in analyzing internal Global Finance Standards documents"

# CO-STAR Framework Components
SUBAGENT_OBJECTIVE = """
To analyze provided Global Finance Standards document sections against a user query and generate an internal research report for the Summarizer agent.
Your objective is to:
1. Determine the relevance of the provided document content (related to currency reporting, resident/non-resident reporting, FX position accounts, Chart of Accounts, FX rates policy) to the user query.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    ...
    prompt_parts = [
        ...
        "<CONTEXT>",
        "You are analyzing sections from the internal Global Finance Standards database.",
        ...
        "</INPUT_DOCUMENTS>",
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the provided Global Finance Standards document section content. Determine how well the content addresses the query.",
...
# Note: Internal Global Finance Standards has multiple documents, so this synthesis prompt is appropriate.
