...
"""
Internal PAR Subagent

This module handles queries to the Internal PAR database, including catalog retrieval,
document selection, content retrieval, and response synthesis.

Functions:
    query_database: Query the Internal PAR database
"""
...
def fetch_par_catalog(query: str) -> List[Dict[str, Any]]:
    """
    Fetch and filter the internal PAR catalog from the database.
    ...
    """
    logger.info(
        f"Fetching PAR catalog with query: {query} (environment: {ENVIRONMENT})"
    )
    ...
    try:
        # Create SQL query to get catalog entries for internal_par
...
def synthesize_response(
    ...
    if not documents:
        error_msg = "No relevant information found in the internal PAR database. Please try refining your query or check other databases."
        if stream:
    ...
    try:
        ...
        error_msg = f"Error synthesizing response from internal PAR documents: {str(e)}"
...
def query_database(
    ...
    """
    Query the Internal PAR database.

    This function implements the full pipeline for querying the internal PAR:
    1. Fetch and filter the PAR catalog
    ...
    Returns:
        Union[str, Generator[str, None, None]]: Query results from the PAR database,
    ...
    def response_generator():
        try:
            ...
            catalog = fetch_par_catalog(query)
            logger.info(f"Retrieved {len(catalog)} catalog entries")

            if not catalog:
                yield "No documents found in the Internal PAR database. Please try a different query or another database."
                return
            ...
            doc_ids = select_relevant_documents(
                query, catalog, token, database_name="internal_par"
            )
            logger.info(f"Selected {len(doc_ids)} relevant documents")

            if not doc_ids:
                yield "No relevant documents found in the Internal PAR database. Please try refining your query or check other databases."
                return
            ...
            response_stream = synthesize_response(
                query, documents, token, stream=True, database_name="internal_par"
            )
