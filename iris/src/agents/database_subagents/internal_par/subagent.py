...
def fetch_par_catalog() -> List[Dict[str, Any]]:
    ...
    """
    logger.info(f"Fetching full PAR catalog (environment: {ENVIRONMENT})")
...
def select_relevant_documents(
    ...
    """
    Use an LLM to select the most relevant PAR documents synchronously.
    """
    ...
    try:
        ...
        try:
            ...
            if isinstance(selected_ids, list) and all(
                ...
                logger.error(
                    f"LLM response for PAR selection was valid JSON but not list of strings: {response_str}"
                )
                return []
        except json.JSONDecodeError:
            logger.error(
                "Failed to parse PAR selection LLM response as JSON, attempting fallback"
            )
            matches = re.findall(r'"([^"]+)"', response_str)
            valid_ids = [
                m for m in matches if m.isdigit()
            ]  # Assuming PAR IDs are numeric strings
            if valid_ids:
                logger.warning(
...
def query_database_sync(query: str, scope: str, token: Optional[str] = None, process_monitor=None) -> SubagentResult:
    ...
    try:
        ...
        logger.info(f"LLM selected {len(selected_doc_ids)} relevant PAR document IDs: {selected_doc_ids}")
