...
def select_relevant_documents(
    ...
    token: Optional[str] = None,
    database_name: str = "internal_process_and_controls", # Default to process_controls
    ...
    try:
        ...
        try:
            ...
            if valid_ids:
                ...
                return valid_ids
            logger.error("Could not extract Process and Controls document IDs from response using fallback.")
            return []
    except Exception as e:
...
def query_database_sync(
    ...
    """
    logger.info(
        f"Querying Internal Process and Controls database (sync): '{query}' with scope: {scope}"
    )
    ...
    try:
        ...
        catalog = fetch_process_controls_catalog() # Use process_controls function
        ...
        if scope == "metadata":
            ...
            logger.info(
                f"Returning {len(selected_items)} selected Process and Controls metadata items."
            )
            ...
            documents = fetch_document_content(selected_doc_ids) # Use process_controls function
            logger.info(
