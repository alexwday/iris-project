# internal_process_and_controls/content_synthesis_prompt.py
"""
Prompt templates for synthesizing content AND status from retrieved Process and Controls documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple Process and Controls documents and providing a status summary.
...
SUBAGENT_ROLE = "an expert research assistant specializing in analyzing internal Process and Controls policy documents"

# CO-STAR Framework Components
SUBAGENT_OBJECTIVE = """
To analyze provided Process and Controls document sections against a user query and generate an internal research report for the Summarizer agent.
Your objective is to:
1. Determine the relevance of the provided document content (related to GL Naming Convention, Intra-group Accounts, ICFR, ICMP) to the user query.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    ...
    prompt_parts = [
        ...
        "<CONTEXT>",
        "You are analyzing sections from the internal Process and Controls policy database.",
        ...
        "</INPUT_DOCUMENTS>",
        "<INSTRUCTIONS>",
        "1. **Analyze Relevance:** Carefully read the user query and the provided Process and Controls document section content. Determine how well the content addresses the query.",
...
# Note: Internal Process and Controls has multiple documents, so this synthesis prompt is appropriate.
