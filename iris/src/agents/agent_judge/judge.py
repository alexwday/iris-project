...
def evaluate_research_progress(
    research_statement, completed_queries, remaining_queries, token
):
    """
    ...
    try:
        ...
        # Database information is already included in the SYSTEM_PROMPT via get_full_system_prompt
        # database_content = "Available Databases Information:\n" # Removed
        # for db_id, db_info in AVAILABLE_DATABASES.items(): # Removed
        #     database_content += f"\n{db_info['name']} ({db_id}):\n" # Removed
        ...
        # database_message = {"role": "system", "content": database_content} # Removed
        # messages.append(database_message) # Removed
        ...
        if not remaining_queries and action != "stop_research":
            ...
            logger.warning(
                "Judge chose to continue but no queries remain; forcing stop_research"
            )
