...
"""
Prompt templates for synthesizing content AND status from retrieved ESG documents.

This module contains prompts used to guide the LLM in synthesizing
content from multiple ESG documents and providing a status summary.
...
def get_content_synthesis_prompt(user_query: str, formatted_documents: str) -> str:
    """
    Generate a prompt for synthesizing content AND status from retrieved ESG documents.

    Args:
        user_query (str): The original user query from the research statement
        formatted_documents (str): The formatted content of retrieved ESG document sections
...
# Note: Internal ESG doesn't seem to have an 'individual file synthesis' prompt.
