...
- Building master reference indices from database query results
- Processing [REF:X] markers in streaming output
- Replacing reference markers with clickable href links

Functions:
    build_master_reference_index: Consolidate reference indices from multiple databases
    process_reference_buffer: Smart buffering for reference replacement during streaming
    process_final_references: Replace all remaining [REF:X] markers in final output
"""
...
REF_INDIVIDUAL_PATTERN = re.compile(r"\[REF:(\d+)\]")
REF_LEGACY_PATTERN = re.compile(r"\[REF:([\d,\s\-]+)\]")
REF_INCOMPLETE_PATTERN = re.compile(r"\[REF:?\d*$")


def _is_structured_format(ref_index: Dict[str, Any]) -> bool:
    """
    ...
    Returns:
        True if structured format, False if legacy format.
    """
...
def _build_link_text(
    ...
    Returns:
        Formatted link text like "Filename, Ch. 5, Pg. 10" or "Filename, Pg. 10".
    """
    display_page = (
...
def _build_href(
    ...
    Returns:
        HTML anchor tag with javascript:window.maven.openPdf() call.
    """
    ...
    page_reference = ref_data.get("page_reference", str(page))
    chapter_number = ref_data.get("chapter_number", "")
    source_filename = ref_data.get("source_filename", doc_name)

    s3_url = f"{s3_base_path}/{file_name}"
    link_text = _build_link_text(source_filename, chapter_number, page_reference, page)

    # Escape any quotes in highlight_text to prevent injection
    safe_highlight = highlight_text.replace('"', '\\"').replace("'", "\\'")

    return f'<a href=\'javascript:window.maven.openPdf("{s3_url}", {page}, "{safe_highlight}")\'>{link_text}</a>'


def _parse_ref_ids(ref_text: str) -> List[str]:
    """
    ...
    Returns:
        List of individual reference ID strings.
    """
    ref_ids: List[str] = []
...
def _generate_links_for_refs(
    ...
    Returns:
        Tuple of (list of href strings, list of found ref IDs).
    """
    ...
    for ref_id in ref_ids:
        ...
        if deduplicate_by_page:
            ...
            if page_key not in page_links:
                page_links[page_key] = _build_href(ref_data, config.S3_BASE_PATH)
        else:
            # Generate link even if duplicate page
            href = _build_href(ref_data, config.S3_BASE_PATH)
            page_key = (ref_id, 0)  # Unique key per ref
            page_links[page_key] = href

    if missing_refs:
        logger.warning(f"References not found in index: {missing_refs}")

    return list(page_links.values()), found_refs


def build_master_reference_index(
    ...
    Returns:
        Tuple containing:
            - master_reference_index: Consolidated reference index with sequential IDs.
                Maps ref_id -> {doc_name, file_link, page, chapter_number, etc.}
            - updated_research: Updated aggregated_detailed_research with REF tags replaced.
    """
    ...
    for db_name, ref_index in all_reference_indices.items():
        ...
        if _is_structured_format(ref_index):
...
def _replace_refs_in_text(
    ...
    Returns:
        Text with markers replaced by HTML links.
    """
    ...
    def replace_match(match: re.Match) -> str:
        ...
        ref_ids = _parse_ref_ids(ref_text)
        links, found_refs = _generate_links_for_refs(ref_ids, reference_index)

        if links:
            logger.debug(
                f"Replaced {match.group(0)} with {len(links)} link(s) for refs: {found_refs}"
            )
            return f"\n\n{' '.join(links)}\n\n"
        else:
            return match.group(0)

    # Use a single comprehensive pattern that handles all formats
    # This avoids the double-processing bug
    combined_pattern = re.compile(r"\[REF:([\d,\s\-]+)\]")
    return combined_pattern.sub(replace_match, text)


def process_final_references(
    ...
    Yields:
        Processed content with [REF:X] markers replaced by HTML href links.
    """
    if not buffer:
        return
    ...
    processed = _replace_refs_in_text(buffer, reference_index)
    yield processed


def process_reference_buffer(
    ...
    Returns:
        Tuple of (processed_content_to_output, remaining_buffer).
            - processed_content_to_output: Content ready to yield with refs replaced
            - remaining_buffer: Incomplete content to carry to next chunk
    """
    if not buffer:
        return "", ""

    # Find all complete reference patterns
    all_matches = list(REF_LEGACY_PATTERN.finditer(buffer))

    if not all_matches:
        # No complete references found
        # Check for incomplete reference at the end
        ...
        if len(buffer) < buffer_size:
            # Buffer not full and no incomplete refs - check for potential start
            if buffer.endswith("["):
                return buffer[:-1], "["
            return buffer, ""
        else:
            # Buffer full, no refs - output most, keep small tail for safety
            ...
            if potential_ref_start != -1 and potential_ref_start > len(buffer) - 15:
                ...
                # Keep a small tail in case a ref is split across chunks
                keep_chars = min(10, len(buffer) // 3)
                if keep_chars > 0:
                    return buffer[:-keep_chars], buffer[-keep_chars:]
                return buffer, ""

    # Find the position after the last complete reference
    last_ref_end = max(m.end() for m in all_matches)

    # Check if there's an incomplete reference after the last complete one
    trailing = buffer[last_ref_end:]
    incomplete_in_trailing = REF_INCOMPLETE_PATTERN.search(trailing)

    if incomplete_in_trailing:
        # There's an incomplete reference starting in the trailing content
    ...
    # Replace all complete references in the content to process
    processed = _replace_refs_in_text(content_to_process, reference_index)
