...
This module provides timing, token usage, and cost tracking for IRIS pipeline
stages. It records when each stage (router, clarifier, planner, etc.) starts
and ends, accumulates LLM call metrics, and persists the data to PostgreSQL
for observability and cost analysis.

Used by the main orchestration layer to wrap agent calls. Enable monitoring
via set_process_monitoring_enabled(True) before pipeline execution. Metrics
are written to the process_monitor_logs table at the end of each request.
"""
...
class ProcessMonitoringManager:
    ...
    def _extract_custom_metadata(self, details: Dict[str, Any]) -> Optional[str]:
    ...
    def log_to_database(self, session: Any) -> None:
        ...
        insert_query = text("""
            INSERT INTO process_monitor_logs (
                ...
                decision_details, error_message, custom_metadata
            ) VALUES (
                :run_uuid, :model_name, :stage_name, :stage_start_time, :stage_end_time,
                :duration_ms, :llm_calls, :total_tokens, :total_cost, :status,
                :decision_details, :error_message, :custom_metadata
        ...
        for stage in self.stages.values():
            try:
                ...
                records.append({
                    ...
                    "custom_metadata": self._extract_custom_metadata(stage.details),
                })
...
def set_process_monitoring_enabled(enabled: bool = True) -> None:
    ...
    Args:
        enabled: Whether to enable monitoring for subsequent pipeline runs.
    """
...
def get_process_monitor_instance() -> ProcessMonitoringManager:
    ...
    Returns:
        The shared monitor instance used across all pipeline stages.
    """
