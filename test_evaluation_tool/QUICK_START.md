# Test Evaluation Tool - Quick Start Guide

This quick start guide provides step-by-step instructions to get started with the Test Evaluation Tool.
...
2. **Install dependencies**:
   ```bash
   cd /path/to/test_evaluation_tool
...
### Evaluating a Single Excel File

```bash
# Using the command-line tool
evaluate-tests --excel_dir /path/to/excel/directory --output_dir ./results

# Using the example script
python -m test_evaluation_tool.examples.evaluate_single_file /path/to/excel/file.xlsx --output_dir ./results
```
...
```bash
# Using the command-line tool
evaluate-tests --excel_dir /path/to/excel/directory --recursive --output_dir ./results

# Using the example script
python -m test_evaluation_tool.examples.batch_evaluation /path/to/excel/directory --recursive --output_dir ./results
```
...
1. **Markdown files** of the converted Excel data
2. **Evaluation JSON files** for each test result
3. **Summary files** (when processing multiple files):
...
3. **Excel Processing Errors**:
   - Ensure Excel files follow the expected format (columns A, B, C)
